    ...
    <div class="container">
        ...
        <div class="action-bar">
            <form class="client-search-bar" method="get" action="" style="gap:12px;">
                ...
                <select name="plateforme" style="min-width:120px;">
            ...
            <?php if ($_SESSION['role'] === 'Admin'): ?>
                <button class="add-btn" id="add-client">
            ...
            if (isset($idpartenaire)) {
                ...
                if (empty($Clients)) {
                    ...
                    foreach ($ClientsToShow as $client) {
                        ...
                        echo '<tr>';
                        echo '<td class="client-name-cell"><a href="../clientdetail/clientdetail.php?idclient=' . $client['idclients'] . '" class="client-link"><i class="fas fa-user"></i> <span class="client-name-text">' . htmlspecialchars($client['Nom']) . '</span></a></td>';
                        ...
                        echo '<a href="../clientdetail/clientdetail.php?idclient=' . $client['idclients'] . '" class="btn-card-details modern" title="Détails"><i class="fas fa-eye"></i></a> ';
                        if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin') {
                            echo '<button class="btn btn-icon btn-edit modern" '
                                ...
                                . 'data-client-plateformeurl="' . (isset($client['PlateformeURL']) ? htmlspecialchars($client['PlateformeURL']) : '') . '" '
                                . 'onclick="openEditModal(this)">'
                                . '<i class="fas fa-edit"></i>'
                                . '</button> ';
                            echo '<button class="btn btn-icon btn-delete modern" data-client-id="' . $client['idclients'] . '" onclick="confirmDelete(' . $client['idclients'] . ')"><i class="fas fa-trash-alt"></i></button>';
                        }
                        echo '</td>';
                        echo '</tr>';
                    }
            ...
            <div class="modal-content">
                ...
                <form id="editClientForm" method="POST">
                    ...
                    <div class="form-row" id="edit-tenant" style="display: none;">
                        ...
                        <select class="form-select" id="edit-tenant_value" name="Tenant" onchange="updateEditPlatformURL()">
    <option value="">Choisir un tenant...</option>
    <?php foreach ($clientsHandler->getPlatforms()['Wazo'] as $name => $url): ?>
        <option value="<?= htmlspecialchars($url) ?>"><?= htmlspecialchars($name) ?></option>
    <?php endforeach; ?>
